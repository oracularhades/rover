...
pub fn home_screen() -> impl Widget<HelloState> {
    ...
    let some_else_is_about_to_text = Label::new(
        format!("Your admin wants to control your screen")
    )
    .with_line_break_mode(LineBreaking::WordWrap)
    // .with_font(UI_FONT_BOLD)
    .with_text_size(24.0);

    let if_this_is_your_personal_device_text = Label::new(format!("Your admin would like to view and control your screen"))
    .with_line_break_mode(LineBreaking::WordWrap)
    .with_text_size(14.0);

    let continue_and_lose_control_button = Button::new("Allow access").padding(10.0).on_click(move |_event, data: &mut i64, _env| {
        *data = 1;
    })
